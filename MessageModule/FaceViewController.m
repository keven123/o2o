...
- (void)viewDidLoad {
    [super viewDidLoad];
    UINavigationBar *navigationBar = [[UINavigationBar alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, NAV_HEIGHT + STATUS_BAR_HEIGHT)];
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc]initWithTitle:@"取消" style:UIBarButtonItemStyleBordered target:self action:@selector(dismissMyselfAction:)];
    UINavigationItem *navigationItem = [[UINavigationItem alloc]initWithTitle:@"表情"];
    [navigationItem setRightBarButtonItem:rightItem];
    [navigationBar pushNavigationItem:navigationItem animated:YES];
    [self.view addSubview:navigationBar];
    
    _faceScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, NAV_HEIGHT + STATUS_BAR_HEIGHT, self.view.frame.size.width, self.view.frame.size.height - NAV_HEIGHT - STATUS_BAR_HEIGHT)];
